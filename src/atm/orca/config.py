...
def _proximo_caminho_livre(pasta, nome_arquivo):
    """Retorna nome/caminho sem colisao: se nao existe, usa o nome original; se existe, gera sufixo _v2, _v3..."""
    import os as _os
    base, ext = _os.path.splitext(str(nome_arquivo))
    candidato_nome = str(nome_arquivo)
    candidato_path = _os.path.join(str(pasta), candidato_nome)
    if not _os.path.exists(candidato_path):
