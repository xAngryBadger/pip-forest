    ...
    while True:
        ...
        if v == "1":
            ...
            if df_scope is None or df_scope.empty:
                ...
                continue
            stg_path = os.path.join(cfg.get("input_dir", INPUT_DIR), STG_FILENAME)
            ct_atividades = []
            if os.path.exists(stg_path):
                stg_tarifas = carregar_stg_tarifas(stg_path)
                ct_atividades = sorted(stg_tarifas.keys())
            micro_atividades = sorted(df_scope["atividade"].unique().tolist())
            todas_atividades = sorted(set(ct_atividades + micro_atividades))
            catalogo_scope = _catalogo_atividades_completo(
                [], cfg=cfg, atividades_catalogo=todas_atividades
            )
