...
def to_float(
    ...
    *,
    ...
    """
    Parse float from various formats (BR, EN, JSON, currency).
    
    Args:
        v: Value to parse (int, float, str, None)
        default: Default value on parse failure (used when allow_none=False)
        allow_none: If True, return None on failure instead of default
        strip_currency: If True, strip R$, spaces, and common currency symbols
        log_errors: If True, log warnings on parse failure
    
    Returns:
        float or None (if allow_none=True and parsing fails)
    """
    import logging
    ...
    # Handle BR format (1.234,56) vs EN format (1,234.56)
    if "," in s and "." in s:
        # Both present: assume BR format with thousand separator
        s = s.replace(".", "").replace(",", ".")
    elif "," in s:
        # Only comma: could be BR decimal or EN thousand separator
        # Heuristic: if comma is near end and followed by 2 digits, treat as decimal
        parts = s.split(",")
    ...
    try:
        ...
        if log_errors:
            logger.warning("to_float: failed to parse %r, returning %s", v, default if not allow_none else None)
        return None if allow_none else float(default)


# Aliases for backward compatibility
def _to_float_br(v, default=0.0):
    """Legacy alias: parse BR/EN format, return default on failure."""
    return to_float(v, default=default, allow_none=False, strip_currency=False, log_errors=False)


def _to_float_json(v, default=None):
    """Legacy alias: parse JSON/currency format, return default (None) on failure."""
    return to_float(v, default=default, allow_none=True, strip_currency=True, log_errors=True)


def _to_float_any(v):
    """Legacy alias: parse any format, return None on failure."""
    return to_float(v, default=None, allow_none=True, strip_currency=True, log_errors=False)
