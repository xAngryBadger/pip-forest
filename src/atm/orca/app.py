...
from .config import (
    ...
    carregar_config, modo_somente_hh, _proximo_caminho_livre,
)
...
def _aplicar_filtro_regiao(df, modo_auto=False):
...
def _aplicar_filtro_empresa_e_escopo(df, modo_auto=False):
...
def _selecionar_talhoes_fazenda(df_faz, fazenda):
    ...
    if "metodologia" in df_work.columns:
        ...
        if not metodologias:
            ...
            if confirmar(f"Usar metodologia unica '{metodologias[0]}'?", default=True):
    ...
    if len(talhoes) == 1:
        if confirmar(f"Usar talhao unico '{talhoes[0]}'?", default=True):
            ...
            # Fall through to selection menu below
            pass
...
def _executar_scheduler_fazenda_interativo(cfg, df_scope, faz, catalogo_scope):
    ...
    while True:
        ...
        # NEW: Offer activity adjustment using FULL catalog (CT + micro + de_para)
        catalogo_completo = _catalogo_atividades_completo(
            [], cfg=cfg, atividades_catalogo=catalogo_scope
        )
        if confirmar("Ajustar atividades do escopo (adicionar/remover/substituir)?", default=False):
            df_faz = _menu_ajustar_escopo_atividades(df_faz, cfg, catalogo_completo)
        resultado = calcular_cronograma_inteligente(
            ...
            avaliar_terreno_fn=avaliar_terreno,
        )
