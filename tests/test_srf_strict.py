...
class TestResolverStrict(unittest.TestCase):
    def test_rendimento_strict_miss_returns_none(self):
        ...
        self.assertIsNone(srf.resolver_rendimento_hh(cfg, t, "missing", strict=True))

    def test_rendimento_strict_zero_returns_none(self):
        cfg = {}
        t = {"a": {"rendimento_hh": 0.0}}
        self.assertIsNone(srf.resolver_rendimento_hh(cfg, t, "a", strict=True))
    ...
    def test_somente_bloqueado(self):
        ...
        bloq = {"PLANTIO"}
        self.assertTrue(srf._somente_bloqueado_restante(dg, bloq))
        dg2 = {("T1", "PLANTIO"): 5.0, ("T2", "ROCADA"): 1.0}
        self.assertFalse(srf._somente_bloqueado_restante(dg2, bloq))

    def test_rendimento_strict_mecanizada_returns_zero(self):
        cfg = {}
        t = {"SUBSOL": {"rendimento_hh": 0.0, "rendimento_hm": 1.2, "tipo": "Mecanizada"}}
        self.assertEqual(srf.resolver_rendimento_hh(cfg, t, "SUBSOL", strict=True), 0.0)

    def test_rendimento_strict_zero_non_mecanizada_returns_none(self):
        cfg = {}
        t = {"MANUAL": {"rendimento_hh": 0.0, "tipo": "Manual"}}
        self.assertIsNone(srf.resolver_rendimento_hh(cfg, t, "MANUAL", strict=True))


class TestNormalizarChave(unittest.TestCase):
    def test_removes_accents_and_punct(self):
        self.assertEqual(srf.normalizar_chave("ROÇADA MANUAL Impl. PL APP/RL I"),
                         "rocada manual impl pl app rl i")

    def test_collapses_spaces(self):
        self.assertEqual(srf.normalizar_chave("  FOO   BAR  -  BAZ  "),
                         "foo bar baz")

    def test_empty_string(self):
        self.assertEqual(srf.normalizar_chave(""), "")
