...
def modulo_otimizacao_financeira(cfg, df):
    ...
    for atv, area in sorted(agg.items()):
        ...
        if tarifa_manual:
            ...
            if tarifa_mec:
                ...
                if pct > 15 and area > 2: recomendacoes.append({"atividade": atv, "area": area, "economia": economia, "pct": pct})
            else:
    ...
    if recomendacoes:
        ...
        for rec in sorted(recomendacoes, key=lambda x: -x["economia"]):
            print(f"  {C.GREEN}{B.BULLET}{C.RESET} {rec['atividade'][:40]:<40} {C.CYAN}{rec['area']:.1f} ha{C.RESET} -> {C.YELLOW}R$ {rec['economia']:,.2f} ({rec['pct']:.0f}%){C.RESET}")
        print(f"  {C.DIM}Criterio: economia >15% e area >2 ha{C.RESET}")
    pause()
...
def buscar_arquivos_excel(extensoes=['.xlsx', '.xls', '.csv'], max_profundidade=3, diretorio=None):
    """Busca recursivamente por arquivos Excel/CSV em pastas comuns."""
    if diretorio is None:
        diretorio = os.path.expanduser("~")

    pastas_busca = [
        DIR,  # Diretório do script
    ...
    for pasta_base in pastas_busca:
        ...
        try:
            for raiz, dirs, arquivos in os.walk(pasta_base):
                ...
                profundidade = raiz[len(pasta_base):].count(os.sep)
                ...
                for arquivo in arquivos:
                    if any(arquivo.endswith(ext) for ext in extensoes):
                        ...
                        # Chave amigável: caminho relativo simpificado
                        chave = f"{arquivo}  ({os.path.dirname(caminho_completo)})"
...
def picker_arquivo(titulo="SELECIONE UM ARQUIVO", extensoes=['.xlsx', '.xls', '.csv'], arquivo_default=None):
    ...
    print(f"\n  {C.BOLD}{C.WHITE}Procurando arquivos...{C.RESET}")
    arquivos = buscar_arquivos_excel(extensoes)

    if not arquivos:
        msg_error("Nenhum arquivo encontrado nas pastas padrão.")
        print(f"  {C.DIM}Pastas buscadas: Home, Downloads, Documents, Desktop, Atual{C.RESET}\n")
...
def calcular_item(area_ha, colaboradores, rend_hh, jornada_ef, eficiencia=1.0):
    ...
    dias_exatos = horas_nec / cap_dia; dias_int = math.ceil(dias_exatos)
    ...
    return dict(area_ha=area_ha, colaboradores=colaboradores, rend_hh=rend_hh, eficiencia=eficiencia, rend_ef=rend_ef, jornada=jornada_ef, horas_nec=horas_nec, cap_dia=cap_dia, dias_exatos=dias_exatos, dias_int=dias_int, saldo_h=saldo_h, pct_uso=pct_uso, ha_extra=ha_extra)
...
def calcular_ut(df_ut, equipe, cfg):
    ...
    dias_ut = max(it["dias_int"] for it in itens_validos); saldo_total_hh = 0.0
...
def modulo_orcar_fazenda(cfg, df):
    ...
    draw_status_box({"UTs": str(len(uts)), "Atividades distintas": str(len(ativs))})
    ...
    resultados_ut = []
    for chave in uts:
        df_ut = df_faz[df_faz["chave"] == chave]; area_ut = df_ut["area_ha"].iloc[0]
        itens, dias_ut, saldo_hh = calcular_ut(df_ut, equipe, cfg)
        resultados_ut.append({"chave": chave, "area_ha": area_ut, "itens": itens, "dias_ut": dias_ut, "saldo_hh": saldo_hh})
...
def modulo_sprint(cfg, df):
    ...
    print(f"  {C.GREEN}[2]{C.RESET} {C.CYAN}Uma UT especifica{C.RESET}")
    escopo = prompt("Escolha", "1")
    if escopo == "2":
        ut_sel = selecionar("SELECIONE A UT", uts)
        if ut_sel is None: return
        df_alvo = df_faz[df_faz["chave"] == ut_sel]; titulo_alvo = f"UT {ut_sel}"
...
def exibir_relatorio_fazenda(fazenda, resultados_ut, equipe):
    ...
    for rut in resultados_ut:
        print(f"\n  {C.DARK_GREEN}{B.tl}{B.h} {C.BOLD}{C.WHITE}UT: {rut['chave']}{C.RESET} {C.DARK_GREEN}{B.h * 20} {C.CYAN}{rut['area_ha']:.3f} ha{C.RESET} {C.DARK_GREEN}{B.h * 20}{B.tr}{C.RESET}")
        ...
        print(f"  {C.GRAY}Duracao da UT: {C.BOLD}{rut['dias_ut']}d{C.RESET}  |  {C.GRAY}Saldo HH ocioso: {C.YELLOW}{rut['saldo_hh']:.1f}h{C.RESET}")
        total_dias = max(total_dias, rut["dias_ut"])
        total_hh += sum(it["horas_nec"] for it in rut["itens"] if not it.get("sem_equipe"))
        total_ha += rut["area_ha"]; total_saldo_hh += rut["saldo_hh"]
    print(f"\n  {C.BOLD}{C.WHITE}CONSOLIDADO - {fazenda}{C.RESET}"); draw_line(B.H)
    draw_status_box({"UTs analisadas": str(len(resultados_ut)), "Area total (ha)": f"{total_ha:.3f}", "Total horas-homem": f"{total_hh:.1f} h", "Saldo HH ocioso total": f"{total_saldo_hh:.1f} h", "PRAZO ESTIMADO": f"{total_dias} dia(s) uteis"})
    print(f"  {C.DIM}(Prazo = UT mais longa; atividades distintas trabalham em paralelo){C.RESET}")
...
def exportar_txt(fazenda, resultados_ut, equipe):
    ...
    for rut in resultados_ut:
        L += [f"\n  UT: {rut['chave']}  |  {rut['area_ha']:.3f} ha", sep2, f"  {'ATIVIDADE':<48} {'HA':>7} {'HH':>7} {'DIAS':>5}", sep2]
        for it in rut["itens"]:
        ...
        L += [sep2, f"  {'Duracao da UT':>50}  {rut['dias_ut']:>3}d", f"  {'Saldo HH ocioso':>50}  {rut['saldo_hh']:>6.1f}h"]
        total_dias = max(total_dias, rut["dias_ut"])
        total_hh += sum(it["horas_nec"] for it in rut["itens"] if not it.get("sem_equipe"))
        total_ha += rut["area_ha"]; total_saldo += rut["saldo_hh"]
    L += ["\n" + sep, "  TOTAIS DA FAZENDA", sep2, f"  UTs                : {len(resultados_ut)}", f"  Area total (ha)    : {total_ha:.3f}", f"  Horas-homem totais : {total_hh:.1f} h", f"  Saldo HH ocioso    : {total_saldo:.1f} h", f"  PRAZO ESTIMADO     : {total_dias} dia(s) uteis", sep, "\n  EQUIPE CONFIGURADA", sep2]
...
def modulo_catalogo(df):
    ...
    headers = ["FAZENDA", "UT", "ATIVIDADE", "HA"]; rows = []
...
def menu(cfg, df):
    while True:
        ...
        draw_status_box({"Base de Dados": f"{nf} Fazendas  {B.BULLET}  {nu} UTs  {B.BULLET}  {na} Atividades", "Configuracoes": f"{nt} Tarifas importadas  {B.BULLET}  {nm} Mapeamentos"})
        print(f"\n  {C.BOLD}{C.WHITE}ORCAMENTO E SIMULACAO{C.RESET}")
        print(f"  {C.GREEN}[1]{C.RESET} {C.WHITE}Orcar fazenda completa{C.GRAY} (UT x atividade x equipe){C.RESET}")
...
def main():
    ...
    if df is None or len(df) == 0:
        ...
        pause()
        return
    msg_ok(f"Dados carregados. {len(df)} registros  |  {df['fazenda'].nunique()} fazendas  |  {df['chave'].nunique()} UTs")
