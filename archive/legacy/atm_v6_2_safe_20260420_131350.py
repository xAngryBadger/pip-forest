"""
SRF — Sistema de Restauracao Florestal v6.2
Smart Scheduler com Comparativo Manual/Mecanizado
Uso : python atm_v6_2.py
ATM_DEMO=1 python atm_v6_2.py
python atm_v6_2.py --demo
    ...
    if isinstance(cfg, dict):
        ...
        if p_cfg:
            ...
            if not os.path.isabs(p_cfg):
                p_cfg = os.path.join(DIR, p_cfg)
            out.append(p_cfg)
    out.append(os.path.join(DIR, PRECO_FINAL_JSON_DEFAULT))
...
def modulo_rotas_metas_bonus(cfg):
    dashboard_header()
    subcabecalho("ROTAS DE METAS / BONIFICACAO / EQUACOES (PREPARATORIO)")
    ...
    while True:
        ...
        print(
            ...
            + f"  Formula bonus AA: "
            + C
            + f"{str(m.get('bonus_aa_formula', ''))[:56]}"
            + RS
        )
        print(
            G
            + f"  Formula bonus BB: "
            + C
            + f"{str(m.get('bonus_bb_formula', ''))[:56]}"
            + RS
        )
        print(
            G
            + f"  Equacao quadratica (rota): "
            + C
            + f"{str(m.get('equacao_quadratica', ''))[:56]}"
            + RS
        )
        sub()
        ...
        if op == "1":
            ...
            m["margem_alvo_pct"] = (
                None if not v.strip() else float(str(v).replace(",", "."))
            )
        elif op == "3":
...
def _find_default_ct_path():
    preferido = os.path.join(DIR, CT_REAL_FILENAME)
    if os.path.exists(preferido):
        return preferido

    # Preferencia por CT 317 quando existir com outro nome.
    for f in buscar_arquivos_excel():
        if f == STG_FILENAME:
            continue
        n = remover_acentos(f)
    ...
    # Fallback legado CT 313 para manter compatibilidade.
    for f in buscar_arquivos_excel():
        if f == STG_FILENAME:
            continue
        n = remover_acentos(f)
...
def construir_cronograma_mecanizado_auto_hm_tarifa(
    ...
    for atv in sorted(filas_por_atividade.keys(), key=str):
        ...
        custo_h = resolver_custo_hora(cfg, tarifas, t_nome, strict=False) or 0.0
                ...
                cronograma.append(
                    {
                        ...
                        "HH": round(hm_exec, 2),
                        "HM": round(hm_exec, 2),
                        "Custo_MO": round(hm_exec * float(custo_h), 2),
...
def simular_cenarios_multifator(
    ...
    for j in jornadas:
        for e in equipes:
            ...
            margem = (
                (lucro / float(receita_total) * 100.0)
                if float(receita_total) > 0.01
                else 0.0
            )
...
def calcular_cronograma_inteligente(
    ...
    if _batch:
        ...
        jornada = pedir_float(
            "Jornada efetiva diaria (horas no campo)", round(j_def, 2)
        )
            ...
            while cap_pool > 0.01:
                ...
                for talhao in talhoes_ordenados:
                    ...
                    for t in tlist:
                        ...
                        ch = (
                            resolver_custo_hora(cfg, tarifas, t_nome_cr, strict=False)
                            or 0.0
                        )
                        cronograma.append(
                            {
            ...
            if usar_reforco_automatico and cap_dia > 0.01:
                for talhao in talhoes_ordenados:
                    ...
                    for t in tarefas_t:
                        ...
                        ch_ref = (
                            resolver_custo_hora(cfg, tarifas, t_nome_ref, strict=False)
                            or 0.0
                        )
                        cronograma.append(
                            {
    ...
    cap_total = sum(
        float(dias_simulado_hum) * float(t["operarios"]) * float(jornada)
        for t in turmas
    )
    hh_ociosa = max(0.0, cap_total - hh_agendada_total)
    custo_medio_hora = (
        (custo_mo_efetivo / hh_agendada_total) if hh_agendada_total > 0.01 else 0.0
    )
            ...
            if cronograma_mec and atividades_mec_set:
                ...
                for rec in recursos_mec:
                    t_mec.add_row(
                        ...
                        if MODO_SOMENTE_HH
                        else f"{rec['prod_ha_h']} ha/h | R$ {rec['custo_h']}/h",
                    )
                    t_mec.add_row(
                ...
                custo_mec_total = sum(
                    float(x.get("Custo_MO", 0)) for x in cronograma_mec
                )
    ...
    for turma in turmas:
        ...
        t_occ.add_row(
            ...
            f"{cap:.1f}",
            f"{pct:.0f}%",
            "-" if MODO_SOMENTE_HH else f"R$ {custo_por_turma.get(nm, 0):,.2f}",
        )
    if hh_por_turma.get("Pelotao_Unificado", 0) > 0.01:
        ...
        t_occ.add_row(
            ...
            f"{cap_p:.1f}",
            f"{pct_p:.0f}%",
            "-" if MODO_SOMENTE_HH else f"R$ {cu:,.2f}",
        )
    console.print(t_occ)
    print(
        DM
    ...
    sub()
    if MODO_SOMENTE_HH:
        print(Y + BL + "  PAINEL FINANCEIRO DESATIVADO (MODO SOMENTE HH)" + RS)
        print(DM + "  Este modo exibe apenas atividades, rendimentos e cronograma operacional." + RS)
    else:
        ...
        if custo_global_total > 0.01:
            ...
            t_fin.add_row(
                "Custo Indireto Global (bruto)", f"R$ {custo_indireto_global:,.2f}"
            )
            t_fin.add_row("Custo Global Total", f"R$ {custo_global_total:,.2f}")
            t_fin.add_row(
                "Lucro Operacional (apos custos globais)", f"R$ {lucro_operacional:,.2f}"
            )
        ...
        if n_fb > 0:
            t_fin.add_row(
                "Itens com fallback (sem tarifa CT)",
                f"{pct_fallback:.0f}% ({n_fb}/{n_demandas})",
            )
        t_fin.add_row(
            "Fonte dos dados",
            "100% CT" if pct_fallback < 0.01 else f"{100 - pct_fallback:.0f}% CT",
        )
        metas = cfg.get("metas", {})
        if metas:
            if metas.get("lucro_alvo") is not None:
                t_fin.add_row(
                    "Meta de Lucro (R$)", f"R$ {float(metas.get('lucro_alvo') or 0):,.2f}"
                )
            if metas.get("margem_alvo_pct") is not None:
                t_fin.add_row(
                    "Meta de Margem (%)", f"{float(metas.get('margem_alvo_pct') or 0):.1f}%"
                )
            ...
            if metas.get("equacao_quadratica"):
                t_fin.add_row(
                    "Equacao quadratica (rota)", str(metas.get("equacao_quadratica"))[:38]
                )
        console.print(t_fin)
    ...
    if cronograma_base:
        try:
            ...
            if not MODO_SOMENTE_HH:
                ...
                nome_fin = "DESATIVADO (modo somente HH)"
            ...
            if not df_atv.empty:
                ...
                df_pivot["Margem_%"] = (
                    (df_pivot["Lucro"] / df_pivot["Receita_Orcada"] * 100)
                    .fillna(0)
                    .round(1)
                )
            ...
            rows_resumo = [
                ...
                {
                    "Metrica": "Dossier operacional (cronograma+cascata, Gantt — sem R$)",
                    "Valor": nome_op,
                },
            ...
            if receita_mec > 0.01:
                rows_resumo.append(
                    {
                        "Metrica": "  Receita Mao de Obra",
                        "Valor": f"R$ {receita_mo:,.2f}",
                    }
                )
                rows_resumo.append(
                    {
                        "Metrica": "  Receita Mecanizada",
                        "Valor": f"R$ {receita_mec:,.2f}",
                    }
                )
            rows_resumo += [
                {
                    "Metrica": "Custo MO (Trabalho Ativo)",
                    "Valor": f"R$ {custo_mo_efetivo:,.2f}",
                },
                {
                    "Metrica": "Custo MO (Ociosidade Logistica)",
                    "Valor": f"R$ {custo_mo_ociosidade:,.2f}",
                },
                {
                    "Metrica": "Custo MO Total Equipe",
                    "Valor": f"R$ {custo_mo_total:,.2f}",
                },
                {
                    "Metrica": "Lucro Direto (Receita - Custo MO)",
                    "Valor": f"R$ {lucro_direto:,.2f}",
                },
                {"Metrica": "", "Valor": ""},
                {"Metrica": "HH Total Simulado", "Valor": f"{total_hh:,.1f}"},
                {
                    "Metrica": "Custo/hora TF (CT_313)",
                    "Valor": f"R$ {float(cfg.get('custo_hora_tf', 0)):,.2f}",
                },
                {
            ...
            if custo_global_total > 0.01:
                rows_resumo += [
                    {
                        "Metrica": "Custo Direto Global (bruto)",
                        "Valor": f"R$ {custo_direto_global:,.2f}",
                    },
                    {
                        "Metrica": "Custo Indireto Global (bruto)",
                        "Valor": f"R$ {custo_indireto_global:,.2f}",
                    },
                    {
                        "Metrica": "Custo Global Total",
                        "Valor": f"R$ {custo_global_total:,.2f}",
                    },
                    {
                        "Metrica": "Lucro Operacional (apos custos globais)",
                        "Valor": f"R$ {lucro_operacional:,.2f}",
                    },
            ...
            if metas:
                ...
                rows_resumo.append(
                    {"Metrica": "Rotas de Metas/Bonificacao", "Valor": "Preparatorio"}
                )
                if metas.get("lucro_alvo") is not None:
                    rows_resumo.append(
                        {
                            "Metrica": "Meta Lucro (R$)",
                            "Valor": f"R$ {float(metas.get('lucro_alvo') or 0):,.2f}",
                        }
                    )
                if metas.get("margem_alvo_pct") is not None:
                    rows_resumo.append(
                        {
                            "Metrica": "Meta Margem (%)",
                            "Valor": f"{float(metas.get('margem_alvo_pct') or 0):.1f}%",
                        }
                    )
                if metas.get("bonus_aa_formula"):
                    rows_resumo.append(
                        {
                            "Metrica": "Bonus AA (rota)",
                            "Valor": str(metas.get("bonus_aa_formula")),
                        }
                    )
                if metas.get("bonus_bb_formula"):
                    rows_resumo.append(
                        {
                            "Metrica": "Bonus BB (rota)",
                            "Valor": str(metas.get("bonus_bb_formula")),
                        }
                    )
                if metas.get("equacao_quadratica"):
                    rows_resumo.append(
                        {
                            "Metrica": "Equacao quadratica (rota)",
                            "Valor": str(metas.get("equacao_quadratica")),
                        }
                    )
                    ...
                    rows_resumo.append(
                        {
                            ...
                            "Valor": f"{rec['prod_ha_h']} ha/h | R$ {rec['custo_h']}/h",
                        }
                    )
                    rows_resumo.append(
                        {
            ...
            rows_op = [
                {"Metrica": "Fazenda", "Valor": fazenda},
                {
                    "Metrica": "Dossier financeiro"
                    if MODO_SOMENTE_HH
                    else "Dossier financeiro (valores R$)",
                    "Valor": nome_fin,
                },
                {"Metrica": "Executores", "Valor": executores},
                {"Metrica": "Jornada (h/dia)", "Valor": jornada},
            ...
            if metas_op:
                ...
                if metas_op.get("margem_alvo_pct") is not None:
                    rows_op.append(
                        {
                            "Metrica": "Meta Margem (%)",
                            "Valor": f"{float(metas_op.get('margem_alvo_pct') or 0):.1f}%",
                        }
                    )
                if metas_op.get("bonus_aa_formula"):
                    rows_op.append(
                        {
                            "Metrica": "Bonus AA (rota)",
                            "Valor": str(metas_op.get("bonus_aa_formula")),
                        }
                    )
                if metas_op.get("bonus_bb_formula"):
                    rows_op.append(
                        {
                            "Metrica": "Bonus BB (rota)",
                            "Valor": str(metas_op.get("bonus_bb_formula")),
                        }
                    )
                if metas_op.get("equacao_quadratica"):
                    rows_op.append(
                        {
                            "Metrica": "Equacao quadratica (rota)",
                            "Valor": str(metas_op.get("equacao_quadratica")),
                        }
                    )
            if recursos_mec:
            ...
            df_cascata = _gerar_aba_cascata_explicada(cronograma_base, jornada, dia_ref, mes_ref, ano_ref)
            ...
            df_timeline = _gerar_aba_timeline(cronograma_base, seq_cfg, modo_seq, atividades_reais, fazenda, dia_ref, mes_ref, ano_ref)

            if not MODO_SOMENTE_HH and caminho_fin:
                with pd.ExcelWriter(caminho_fin, engine="openpyxl") as writer_fin:
                    ...
                    if not df_pivot.empty:
                        df_pivot.to_excel(
                            writer_fin, sheet_name="CUSTO_POR_ATIVIDADE", index=False
                        )
                    if cenarios_rows:
                        pd.DataFrame(cenarios_rows).to_excel(
                            writer_fin, sheet_name="COMPARATIVO_CENARIOS", index=False
                        )
            ...
            with pd.ExcelWriter(caminho_op, engine="openpyxl") as writer_op:
                ...
                if recursos_mec and cronograma_mec:
                    ...
                    df_combinado = _df_crono_operacional(
                        pd.DataFrame(cronograma_com_mec)
                    )
            ...
            if not MODO_SOMENTE_HH:
                ...
                aviso("Dossier financeiro desativado no modo somente HH.")
            ...
            if recursos_mec and cronograma_com_mec:
                ...
                caminho_mec_fin = None
                if not MODO_SOMENTE_HH:
                    nome_mec_fin, caminho_mec_fin = _proximo_caminho_livre(
                        pasta_dossier, nome_mec_fin
                    )
                else:
                    nome_mec_fin = "DESATIVADO (modo somente HH)"
                ...
                resumo_mec_rows = [
                    {"Metrica": "Fazenda", "Valor": fazenda},
                    {
                        "Metrica": "Dossier operacional (cenario mecanizado)",
                        "Valor": nome_mec_op,
                    },
                ...
                rows_mec_op = [
                    {"Metrica": "Fazenda", "Valor": fazenda},
                    {
                        "Metrica": "Dossier financeiro (cenario mecanizado, R$)",
                        "Valor": nome_mec_fin,
                    },
                    {"Metrica": "Cenario", "Valor": "Humano + Mecanizado"},
                    {"Metrica": "Dias baseline (humano)", "Valor": dias_simulado},
                ...
                if not MODO_SOMENTE_HH and caminho_mec_fin:
                    with pd.ExcelWriter(caminho_mec_fin, engine="openpyxl") as writer_mf:
                        pd.DataFrame(resumo_mec_rows).to_excel(
                            writer_mf, sheet_name="RESUMO_FINANCEIRO", index=False
                        )
                ...
                if not MODO_SOMENTE_HH:
                    ...
                    aviso("Dossier mecanizado financeiro desativado no modo somente HH.")
    ...
    if resultado_mecanizado_valido:
        ...
        rec_manual = float(receita_total)
        rec_mec = float(resultado_mecanizado.get("receita_total") or 0)
        custo_manual = float(custo_mo_total)
        custo_mec = float(resultado_mecanizado.get("custo_mo_total") or 0)
        lucro_manual = float(lucro_operacional)
        lucro_mec = float(resultado_mecanizado.get("lucro_operacional") or 0)
        
        # Calcular economia
        economia_dias = int(d_manual - d_mec)
        economia_hh = hh_manual - hh_mec
        economia_hm = hm_mec - hm_manual
        economia_custo = custo_manual - custo_mec
        ganho_lucro = lucro_mec - lucro_manual
        ...
        print(f"  {'Receita total (R$)':<25} {rec_manual:>10,.0f}      {rec_mec:>10,.0f}      {rec_mec-rec_manual:>+10,.0f}{RS}")
        print(f"  {'Custo MO (R$)':<25} {custo_manual:>10,.0f}      {custo_mec:>10,.0f}      {G}{economia_custo:>+10,.0f}{RS}")
        print(f"  {'Lucro operacional (R$)':<25} {lucro_manual:>10,.0f}      {lucro_mec:>10,.0f}      {G}{ganho_lucro:>+10,.0f}{RS}")
        print()
        if turmas_mec_comp:
        ...
        if economia_custo > 0:
            print(f"  {G}✓{RS} Economia de custo: {G}R$ {economia_custo:,.2f}{RS}")
        if ganho_lucro > 0:
            print(f"  {G}✓{RS} Ganho de lucro: {G}R$ {ganho_lucro:,.2f}{RS}")
        print()
        print(G + BL + "═══════════════════════════════════════════════════════════════════" + RS)
        sub()
    ...
    _emitir_monitor_relatorio(
        ...
        (
            ...
            f"Receita: R$ {float(receita_total):,.2f}\n"
            f"Custo MO: R$ {float(custo_mo_total):,.2f}\n"
            f"Lucro operacional: R$ {float(lucro_operacional):,.2f}"
        ),
    )
    ...
    if resultado_mecanizado_valido:
        resultado_final["comparativo_mecanizado"] = {
            ...
            "receita_total": resultado_mecanizado.get("receita_total"),
            "custo_mo_total": resultado_mecanizado.get("custo_mo_total"),
            "lucro_operacional": resultado_mecanizado.get("lucro_operacional"),
    ...
    try:
        ...
        resumo_rows = [
            ...
            },
            {
                ...
                "Valor": round(
                    sum(float(x.get("receita_total", 0)) for x in resultados), 2
                ),
            },
            {
                "Metrica": "Custo MO total",
                "Valor": round(
                    sum(float(x.get("custo_mo_total", 0)) for x in resultados), 2
                ),
            },
            {
                "Metrica": "Lucro operacional (soma)",
                "Valor": round(
                    sum(float(x.get("lucro_operacional", 0)) for x in resultados), 2
                ),
            },
        ]
        d_mec_vals = [
...
def _executar_lote_fazendas(
    ...
    t_all.add_row(
        "Receita total",
        f"R$ {sum(float(x.get('receita_total', 0)) for x in resultados):,.2f}",
    )
    t_all.add_row(
        "Custo MO total",
        f"R$ {sum(float(x.get('custo_mo_total', 0)) for x in resultados):,.2f}",
    )
    t_all.add_row(
        "Lucro operacional",
        f"R$ {sum(float(x.get('lucro_operacional', 0)) for x in resultados):,.2f}",
    )
    console.print(t_all)
...
def _selecionar_talhoes_fazenda(df_faz, fazenda):
    ...
    if "metodologia" in df_work.columns:
        ...
        if not metodologias:
            ...
            ok(f"Metodologia unica no escopo da fazenda: {metodologias[0]}")
        else:
            print(G + BL + "\n  ESCOPO POR METODOLOGIA" + RS)
            print(DM + f"  Fazenda: {fazenda}" + RS)
            print(DM + f"  {len(metodologias)} metodologia(s) disponivel(is)." + RS)
            op_met = selecionar(
...
def _menu_ajustar_escopo_atividades(df_faz, cfg=None, atividades_catalogo=None):
    ...
    while True:
        ...
        if op == "Substituir atividade":
            ...
            if not src:
                continue
            destinos = [x for x in catalogo_all if x != src]
            dst_opt = selecionar(
                "DESTINO",
                destinos + ["[DIGITAR NOVA ATIVIDADE]"],
            )
            if not dst_opt:
                continue
            dst = (
    ...
    if usar_contrato:
        ...
        try:
            ...
            if stg_path and n > 0:
                ...
                if MODO_SOMENTE_HH:
                    ok(
                        f"CT auto: {os.path.basename(ct_padrao)} -> {n} atividades (modo somente HH)"
                    )
                else:
                    ok(
