...
def menu_vincular_atividades_turma(turma, atividades_reais):
    ...
    while True:
        ...
        if op == "1":
            ...
            origem = cur2[old]
            disp = sorted(
                [a for a in atividades_reais if a != origem], key=lambda x: str(x)
            )
            if not disp:
...
def calcular_cronograma_inteligente(
    cfg, df_faz, fazenda, esperar_enter=True, ctx=None, escopo_meta=None
):
    """
    ...
    cap_total = sum(
        float(dias_simulado) * float(t["operarios"]) * float(jornada) for t in turmas
    )
    ...
    print(
        DM
        + "  Cadastre recursos mecanizados (robo, trator, etc.) que assumem atividades do humano."
        + RS
    )
    if confirmar("  Ativar modo mecanizado?", default=False):
        ...
        if recursos_mec and atividades_mec_set:
            ...
            for rec in recursos_mec:
                t_mec.add_row(
                    f"  Recurso: {rec['nome']}",
                    f"{rec['prod_ha_h']} ha/h | R$ {rec['custo_h']}/h",
                )
                t_mec.add_row(
    ...
    for turma in turmas:
        ...
        t_occ.add_row(
            ...
            f"{cap:.1f}",
            f"{pct:.0f}%",
            f"R$ {custo_por_turma.get(nm, 0):,.2f}",
        )
    ...
    if cronograma:
        try:
            ...
            cronograma, turmas, jornada, dias_simulado, dia_ref, mes_ref, ano_ref
            )
