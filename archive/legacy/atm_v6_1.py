...
def _find_diaria_tf_sheet(xls):
    for s in xls.sheet_names:
        ns = remover_acentos(s).replace(" ", "")
        if "diaria_tf" in ns or ns == "tf":
            return s
    return None
...
def simular_comparativo_robo_rocador(
    demandas, dias_baseline, executores, jornada, prod_ha_h=0.18, custo_h=0.0
):
    """
    ...
    dias_hum_sem_rocada = (
        int(math.ceil(hh_sem_rocada / cap_hum_dia)) if cap_hum_dia > 0 else 0
    )
...
def calcular_cronograma_inteligente(
    ...
    ag_hum_set = {
        ...
        for x in cronograma
        if str(x.get("Atividade", "")).strip()
    }
    ag_mec_set = {
        str(x.get("Atividade", "")).strip()
        for x in cronograma_mec_base
        if str(x.get("Atividade", "")).strip()
    } | {
        str(x.get("Atividade", "")).strip()
        for x in cronograma_mec
        if str(x.get("Atividade", "")).strip()
    }
    ...
    for tarefas in demandas.values():
        for t in tarefas:
            hh_por_atividade[str(t.get("atividade", "")).strip()] += float(
                t.get("hh_total", 0) or 0
            )
